---
title: Bilibili 登錄
description: controller 模式下的 B 站掃碼登錄 API
---

當 `BILIBILI_LOGIN_MODE=controller` 時，通過 `/auth/bilibili` 接口在服務運行後觸發掃碼登錄。
...
創建新的二維碼登錄會話。支持賬號切換，即使已登錄也可重新發起。
...
已登錄用戶可隨時調用 `POST /auth/bilibili/init` 切換到新賬號。
