---
title: 設定示例
description: 完整的 bilirec 環境變數設定示例
---

以下是一份完整的設定示例，可直接複製到 `.env` 檔案或作為 `export` 腳本使用：

```bash
export BILIBILI_LOGIN_MODE=controller
# export HOST=0.0.0.0
export PORT=8080
# export SERVER_CRT=/path/to/server.crt
# export SERVER_KEY=/path/to/server.key
# 監控指標（預設關閉，啟用後由 VictoriaMetrics / Grafana 抓取）
# export METRICS_ENABLED=true
# export METRICS_HOST=          # 留空監聽所有網卡，便於跨機器抓取
# export METRICS_PORT=9090
# VictoriaLogs 遠端日誌（預設關閉）
# export VICTORIALOGS_ENABLED=true
# export VICTORIALOGS_URL=http://127.0.0.1:9428
# export VICTORIALOGS_STREAM_FIELDS=app,logger
# 本地日誌檔案（預設關閉；隊列滿時預設 drop，不阻塞錄製，詳見 local-logs）
# export LOCAL_LOGS_ENABLED=true
# export LOCAL_LOGS_PATH=./logs/bilirec.log
# export LOCAL_LOGS_FORMAT=pretty
# export LOCAL_LOGS_OVERFLOW=drop
export FRP_ENABLED=true
export FRP_SERVER=tunnel.bilirec.org:7000
export FRP_BASE_DOMAIN=tunnel.bilirec.org
export TRUSTED_PROXIES=161.33.159.26
export FRP_HTTPS=false
export FRP_SCHEME_HTTPS=true
export MAX_CONCURRENT_RECORDINGS=3
export MAX_RECORDING_HOURS=5
export MAX_RECOVERY_ATTEMPTS=15
export MAX_RETRY_MINUTES=10
export RECORDING_RECOVERY_DURATION=preserve
# export SUBCHECK_ROOMS_PER_SHARD=50
# export SUBCHECK_TICK_SECS=10
# export SUBCHECK_MIN_INTERVAL_SECS=60
# export SUBCHECK_MAX_INTERVAL_SECS=300
# export SUBCHECK_MAX_SHARDS=32
# export SUBCHECK_JITTER_SECS=2  # 打散輪詢節奏，降低反爬風控
export OUTPUT_DIR=/path/to/records
export SECRET_DIR=/path/to/secrets
export NO_CONVERT_IF_INVALID=false
export DATABASE_DIR=/path/to/database
export CONVERT_TO_MP4=false
export DELETE_SOURCE_AFTER_CONVERT=false
export CLOUDCONVERT_THRESHOLD=1073741824
export CLOUDCONVERT_API_KEY=
export CLOUDCONVERT_CHECK_INTERVAL_SECS=180
export CLOUDCONVERT_MAX_CONCURRENT_DOWNLOADS=1
export CLOUDCONVERT_ALLOW_DURING_RECORDING=false
export CLOUDCONVERT_ALLOW_DURING_RECORDING_MAX_ACTIVE_RECORDINGS=1
export FFMPEG_CHECK_INTERVAL_SECS=60
export FFMPEG_MAX_CONCURRENT_TASKS=1
export FFMPEG_ALLOW_DURING_RECORDING=false
export FFMPEG_ALLOW_DURING_RECORDING_MAX_ACTIVE_RECORDINGS=1
export PUBLIC_BASE_URL=http://localhost:8080
export FRONTEND_URL=https://app.bilirec.org
export WEBPUSH_SUBSCRIBER=mailto:webpush@example.com
export UPLOAD_BUFFER_SIZE=5242880
export DOWNLOAD_BUFFER_SIZE=5242880
export STREAM_WRITER_BUFFER_SIZE=1048576
export READ_STREAM_BYTES_POOL_SIZE=524288
export READ_STREAM_CHAN_BUFFER_SIZE=16
export READ_STREAM_BYTES_POOL_SIZE_HIGH=1048576
export READ_STREAM_CHAN_BUFFER_SIZE_HIGH=48
export LIVE_STREAM_WRITER_BUFFER_SIZE=8388608
export LIVE_STREAM_WRITER_SYNC_PERIOD_SECS=0
export LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS=60
export LIVE_STREAM_WRITER_FLUSH_PERIOD_SECS=15
export LIVE_STREAM_WRITER_CHAN_BUFFER_SIZE=64
export LIVE_STREAM_WRITER_BYTES_POOL_SIZE=524288
export LIVE_STREAM_WRITER_BYTES_POOL_SIZE_HIGH=1048576
export SKIP_SMALL_FLUSH_THRESHOLD=1048576
export SKIP_SMALL_FLUSH=true
export SEQUENTIAL_WRITE=true
# 預設已開 DROP + 冷釋放；SSD/HDD 高性能場景可關閉：
# export DROP_FILE_PAGE_CACHE=false
# export LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS=0
# 彈幕錄製（是否錄製由房間設定或 record_danmaku 控制）
export DANMAKU_OUTPUT_FORMAT=jsonl
export DANMAKU_OVERFLOW_POLICY=drop
export DANMAKU_WRITER_BUFFER_SIZE=262144
export DANMAKU_WRITER_MIN_PERIODIC_FLUSH_BYTES=16384
export DANMAKU_WRITER_FLUSH_PERIOD_SECS=15
export DANMAKU_WRITER_SYNC_PERIOD_SECS=0
export DANMAKU_CHAN_BUFFER_SIZE=256
export DANMAKU_BYTES_POOL_SIZE=4096
export MIN_DISK_SPACE_BYTES=5368709120
export JWT_SECRET=bilirec_secret
export DEBUG=false
export USERNAME=admin
export PASSWORD=changeme
# 本示例啟用了官方 FRP，並通過 app.bilirec.org 遠端管理，須開啟生產模式
export PRODUCTION_MODE=true
```

各變數的詳細說明見[環境變數參考](/zh-tw/configuration/env-reference/)。針對不同硬件的調優方案見 [樹莓派預設](/zh-tw/configuration/pi5-defaults/)、[高碼率 / 4K 調參](/zh-tw/configuration/high-bitrate-tuning/)、[SSD 調優](/zh-tw/configuration/ssd-tuning/) 與 [HDD/NAS 調優](/zh-tw/configuration/hdd-nas-tuning/)。
