...
| `LIVE_STREAM_WRITER_SYNC_PERIOD_SECS` | 录制中每隔多久强制整文件落盘（秒）；`0`=只在关档时落盘。与 `COLD_CACHE_RELEASE_SECS` 同时大于 0 时以本项为准：每次 fsync 成功后顺带释放冷前缀；`COLD=0` 时仅落盘、不释放缓存 | `0` |
| `LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS` | 录制中多久清理一次旧文件缓存（秒，仅 Linux/Android）；`0`=关闭。默认 `60`，用于压低容器监控曲线；**不是**录制中反复整文件落盘。大于 `0` 时，同一秒数也用于**播放/下载空闲**（最后一次请求结束后等待这么久再释放）；设为 `0` 时播放/下载仍按默认 **60** 秒空闲等待，与关档释放解耦 | `60` |
...
| `DROP_FILE_PAGE_CACHE` | 录制分段结束、本地 ffmpeg 转码成功、或**在线播放/下载最后一次请求结束**（空闲等待见 `LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS`）后，让系统释放对应**整份文件**的缓存（仅 Linux/Android 有效）。**代价：** 下次播放/下载同一文件需重新从磁盘读取，可能变慢。默认 `true`；显式 `false` 可关。与录製中冷缓存释放（`LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS`）独立 | `true` |
| `SEQUENTIAL_WRITE` | 多路录制时轮流写盘，降低同一张盘上的写入峰值；多路写入同一物理磁盘时建议开启。SSD 可关闭以提升并发 | `true` |
