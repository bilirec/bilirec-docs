...
| `LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS` | 录制中多久清理一次旧文件缓存（秒，仅 Linux/Android）；`0`=关闭。默认 `60`，用于压低容器监控曲线；**不是**录制中反复整文件落盘 | `60` |
...
| `DROP_FILE_PAGE_CACHE` | 录制分段结束或本地 ffmpeg 转码成功后，让系统释放对应**整份文件**的缓存（仅 Linux/Android 有效）。**代价：** 下次播放/下载同一文件需重新从磁盘读取，可能变慢。默认 `true`；显式 `false` 可关。与录製中冷缓存释放（`LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS`）独立 | `true` |
