...
| `DROP_FILE_PAGE_CACHE` | 录制分段结束、本地 ffmpeg 转码成功、或在线播放/下载最后一次请求结束后（会等待 `LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS` 秒，如果数值为0则默认为 60 秒），让系统释放对应**整份文件**的缓存（仅 Linux/Android 有效）。**代价：** 下次播放/下载同一文件需重新从磁盘读取，可能变慢。默认 `true`；显式 `false` 可关。与录製中冷缓存释放（`LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS`）独立 | `true` |
