...
| `DROP_FILE_PAGE_CACHE` | 录制分段结束或本地 ffmpeg 转码成功后，提示内核释放对应文件的页缓存（仅 Linux/Android 有效）。**代价：** 下次播放/下载同一文件需重新从磁盘读取，可能变慢。Docker/Linux 默认 `false`；[Android App](https://www.bilirec.org/zh-cn/guides/android/) 内建默认 `true`，可用环境变量覆盖 | `false` |
