...
（单路在途：读 49 MB + 写 65 MB + 落盘 8 MB ≈ 122 MB）
...
- **容器总内存虚高**：写入磁盘后，系统可能额外缓存一份文件，容器监控显示的总用量会远高于上表。默认已开启 `DROP_FILE_PAGE_CACHE=true` 与 `LIVE_STREAM_WRITER_COLD_CACHE_RELEASE_SECS=60`（录製中释放冷前缀、关档释放整文件），监控曲线会更贴近上表 anon 估算。SSD/HDD 若介意回放变慢可显式关闭（见 [I/O 调优](/zh-cn/configuration/io-tuning/)）。
