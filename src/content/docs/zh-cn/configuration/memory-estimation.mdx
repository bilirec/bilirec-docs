...
- **容器总内存虚高**：写入磁盘后，系统可能额外缓存一份文件，容器监控显示的总用量会远高于上表，规划时以上表为准即可。若需压低监控数字且能接受回放略慢，可设 `DROP_FILE_PAGE_CACHE=true`（见 [常见问题](/zh-cn/guides/faq/#drop_file_page_cache-什么时候用代价是什么)）。
